...
function getStudentDetails(item: ApprovalApplication, index: number) {
  ...
  return {
    ...
    sceneLabel: withdrawal ? "退课" : item.scenarioLabel,
        ...
        <section>
          ...
          <div className="rounded-xl border border-[#dbe3ef] p-4">
            ...
            <div className="mt-4 grid grid-cols-5 gap-3 border-t border-[#edf0f4] pt-4 text-sm">
              ...
              <div><p className="text-xs text-[#98a2b3]">优惠总金额</p><p className="mt-1 font-semibold text-[#d85b18]">-¥ {formatMoney(discountAmount)}</p></div>
...
function buildAmountLessonRows(item: ApprovalApplication, type: string, scene: string): AmountLessonRow[] {
  if (type === "特殊退课") {
    const lessons = [7, 8];
    return lessons.map((lesson) => ({ lesson, status: "本次退款", original: 210, discount: 105, consumed: 105, refunded: 0, remaining: 105, highlight: true }));
  }
